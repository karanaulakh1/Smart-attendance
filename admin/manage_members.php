...
foreach($groups as $g){ if($g['table_name'] === $active_group){ $active_group_info = $g; break; } }
if(!$active_group_info){ $active_group = 'students'; foreach($groups as $g){ if($g['table_name']==='students'){ $active_group_info=$g; break; } } }

$member_table = $active_group_info['table_name'];
$att_table    = $active_group_info['attendance_table'];
...
$members = mysqli_query($conn,"SELECT * FROM `$member_table` WHERE student_id LIKE '%$search%' OR name LIKE '%$search%' OR department LIKE '%$search%' ORDER BY id DESC");
$total   = mysqli_query($conn,"SELECT COUNT(*) as c FROM `$member_table`")->fetch_assoc()['c'];

/* EDIT */
$editData = null;
if(isset($_GET['edit'])){
    $eid = (int)$_GET['edit'];
    $eq  = mysqli_query($conn,"SELECT * FROM `$member_table` WHERE id='$eid'");
...
<html lang="en">
<head>
<title>Manage Members — Smart Attendance</title>
...
.search-input{ padding:11px 14px 11px 38px; background:var(--surface2); border:1px solid var(--border); border-radius:12px; color:var(--text); font-family:'DM Sans',sans-serif; font-size:14px; outline:none; width:240px; transition:.15s; }
.search-input:focus{ border-color:var(--accent); width:280px; }
.search-btn{ padding:11px 20px; border:none; border-radius:12px; background:linear-gradient(135deg,var(--accent),#5b8af9); color:#fff; font-family:'DM Sans',sans-serif; font-size:13px; font-weight:700; cursor:pointer; transition:.2s; }
...
.search-meta{ font-size:13px; color:var(--muted); margin-bottom:16px; }
...
thead th{ padding:13px 16px; font-size:11px; font-weight:700; color:var(--muted); text-transform:uppercase; letter-spacing:.8px; text-align:left; white-space:nowrap; background:var(--surface2); border-bottom:1px solid var(--border); }
tbody td{ padding:14px 16px; font-size:14px; border-bottom:1px solid rgba(255,255,255,.03); vertical-align:middle; }
...
.fp-badge{ display:inline-flex; align-items:center; padding:4px 12px; border-radius:50px; font-size:11px; font-weight:700; font-family:'DM Mono',monospace; }
.fp-ok{ background:rgba(34,197,94,.1); color:#4ade80; border:1px solid rgba(34,197,94,.2); }
.fp-none{ background:rgba(100,116,139,.1); color:var(--muted); border:1px solid rgba(100,116,139,.15); }
.action{ display:flex; gap:6px; align-items:center; flex-wrap:nowrap; }
.act-btn{ display:inline-flex; align-items:center; gap:5px; padding:6px 12px; border-radius:8px; font-size:12px; font-weight:600; text-decoration:none; color:#fff; border:none; cursor:pointer; transition:.15s; white-space:nowrap; }
.act-btn:hover{ transform:translateY(-1px); }
.btn-enroll{ background:rgba(59,110,248,.2); color:#93c5fd; border:1px solid rgba(59,110,248,.3); }
.btn-edit{ background:rgba(245,158,11,.15); color:#fbbf24; border:1px solid rgba(245,158,11,.25); }
.btn-delete{ background:rgba(244,63,94,.1); color:#fb7185; border:1px solid rgba(244,63,94,.2); }
.empty{ text-align:center; padding:60px 20px; }
.empty p{ color:var(--muted); font-size:14px; }
/* MODAL */
.modal{ position:fixed; inset:0; background:rgba(0,0,0,.75); display:flex; justify-content:center; align-items:center; z-index:9000; padding:16px; }
.modal-box{ width:680px; max-width:100%; background:var(--surface); border:1px solid var(--border); border-radius:22px; padding:32px; max-height:90vh; overflow-y:auto; animation:popIn .2s ease; }
...
.modal-close{ background:var(--surface2); border:1px solid var(--border); color:var(--muted); width:32px; height:32px; border-radius:8px; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:16px; transition:.15s; text-decoration:none; }
.modal-close:hover{ color:var(--text); }
.modal-grid{ display:grid; grid-template-columns:1fr 1fr; gap:16px; }
.m-input-wrap{ display:flex; flex-direction:column; gap:6px; }
.m-input-wrap label{ font-size:11px; font-weight:700; color:var(--muted); text-transform:uppercase; letter-spacing:.8px; }
.m-input{ padding:12px 14px; background:var(--surface2); border:1px solid var(--border); border-radius:12px; color:var(--text); font-family:'DM Sans',sans-serif; font-size:14px; outline:none; transition:.15s; width:100%; }
.m-input:focus{ border-color:var(--accent); }
.modal-save{ width:100%; margin-top:20px; padding:13px; border:none; border-radius:12px; background:linear-gradient(135deg,var(--accent),#5b8af9); color:#fff; font-family:'DM Sans',sans-serif; font-size:14px; font-weight:700; cursor:pointer; transition:.2s; }
.modal-save:hover{ transform:translateY(-2px); }
@media(max-width:768px){ .mob-bar{ display:flex; } .sidebar{ left:-280px; } .sidebar.on{ left:0; } .main{ margin-left:0; padding:16px; } .page-title{ font-size:22px; } .top-bar{ flex-direction:column; align-items:flex-start; } .search-form{ width:100%; } .search-wrap{ flex:1; } .search-input{ width:100% !important; } .modal-grid{ grid-template-columns:1fr; } .modal-box{ padding:20px; } thead th, tbody td{ padding:11px 10px; font-size:13px; } .name-sub{ display:none; } }
</style>
</head>
<body>
<div class="mob-bar">
    ...
    <div class="table-card">
        <div class="table-wrap">
            <table>
                <thead>
                    <tr>
                        <th>Member ID</th><th>Name</th><th>Contact</th><th>Department / Course</th><th>Year</th><th>Fingerprint</th><th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                <?php
                ...
                <tr>
                    ...
                    <td style="font-size:13px;color:var(--muted);"><?php echo htmlspecialchars($row['phone']); ?></td>
                    <td>
                        <div class="name-cell">
                ...
                <?php endwhile; ?>
                <?php if(!$hasRows): ?>
                <tr><td colspan="7"><div class="empty"><p><?php echo $search ? 'No members match your search.' : 'No members in this group yet.'; ?></p></div></td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div class="modal-box">
        ...
        <form method="POST">
            ...
            <div class="modal-grid">
                <div class="m-input-wrap"><label>Member ID</label><input class="m-input" type="text" name="student_id" value="<?php echo htmlspecialchars($editData['student_id']); ?>"></div>
                <div class="m-input-wrap"><label>Full Name</label><input class="m-input" type="text" name="name" value="<?php echo htmlspecialchars($editData['name']); ?>"></div>
                <div class="m-input-wrap"><label>Email</label><input class="m-input" type="email" name="email" value="<?php echo htmlspecialchars($editData['email']); ?>"></div>
                <div class="m-input-wrap"><label>Phone</label><input class="m-input" type="text" name="phone" value="<?php echo htmlspecialchars($editData['phone']); ?>"></div>
                <div class="m-input-wrap"><label>Department</label><input class="m-input" type="text" name="department" value="<?php echo htmlspecialchars($editData['department']); ?>"></div>
                <div class="m-input-wrap"><label>Course / Designation</label><input class="m-input" type="text" name="course" value="<?php echo htmlspecialchars($editData['course']); ?>"></div>
                <div class="m-input-wrap"><label>Year / Level</label><input class="m-input" type="text" name="year" value="<?php echo htmlspecialchars($editData['year']); ?>"></div>
                <div class="m-input-wrap"><label>Fingerprint ID</label><input class="m-input" type="text" name="fingerprint_id" value="<?php echo htmlspecialchars($editData['fingerprint_id']); ?>"></div>
            </div>
            <button type="submit" name="update_member" class="modal-save">Save Changes</button>
        </form>
    </div>
</div>
<?php endif; ?>
...
function confirmDelete(name){ return confirm('Delete "'+name+'"? This cannot be undone.'); }
</script>
</body>
</html>