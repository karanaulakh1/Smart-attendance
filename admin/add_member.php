...
if(isset($_POST['create_group'])){
    if($admin_role !== 'superadmin'){
        ...
        if($dup->num_rows > 0){
            ...
            // ── ATTENDANCE TABLE — includes in_time, out_time, working_hours ──
            ...
            // Refresh groups list
            $gq2 = $conn->query("SELECT * FROM groups_registry ORDER BY id ASC");
            $groups = [];
            while($g = $gq2->fetch_assoc()) $groups[] = $g;
        }
    }
}
...
foreach($groups as $g){
    ...
    if($cnt) $total_members += (int)$cnt->fetch_assoc()['c'];
}
?>
<!DOCTYPE html>
<html lang="en">
...
/* ALERTS */
.alert{ display:flex; align-items:center; gap:10px; padding:14px 18px; border-radius:14px; margin-bottom:24px; font-size:14px; font-weight:500; animation:slideIn .3s ease; max-width:820px; }
...
.group-item-check{ font-size:10px; font-weight:700; padding:2px 8px; border-radius:50px; }
.check-ok{ background:rgba(34,197,94,.12); color:#4ade80; border:1px solid rgba(34,197,94,.2); }
.check-missing{ background:rgba(244,63,94,.1); color:#fb7185; border:1px solid rgba(244,63,94,.18); }
...
.superadmin-note{ font-size:11px; color:var(--muted); margin-top:8px; line-height:1.6; }

/* IN/OUT NOTICE */
.inout-notice{ display:flex; align-items:flex-start; gap:10px; background:rgba(59,110,248,.08); border:1px solid rgba(59,110,248,.18); border-radius:12px; padding:12px 14px; margin-top:4px; }
.inout-notice-icon{ font-size:16px; flex-shrink:0; margin-top:1px; }
.inout-notice p{ font-size:12px; color:#93c5fd; line-height:1.6; }
                    ...
                    <div class="input-wrap full">
                        ...
                        <select class="f-select" name="group_table" required>
                            <?php foreach($groups as $g){
                                echo '<option value="'.htmlspecialchars($g['table_name']).'">'.htmlspecialchars($g['group_name']).'</option>';
                            } ?>
                        </select>
                    </div>
        ...
        <div class="side-panel">
            ...
            <div class="info-card">
                ...
                <div class="group-list">
                    <?php foreach($groups as $g):
                        $cnt = (int)$conn->query("SELECT COUNT(*) as c FROM `".$g['table_name']."`")->fetch_assoc()['c'];

                        // Check if attendance table has in_time column
                        $col_check = $conn->query("SHOW COLUMNS FROM `".$g['attendance_table']."` LIKE 'in_time'");
                        $has_inout = ($col_check && $col_check->num_rows > 0);
                    ?>
                    <div class="group-item">
                        <span class="group-item-name"><?php echo htmlspecialchars($g['group_name']); ?></span>
                        <div style="display:flex;align-items:center;gap:6px;">
                            <span class="group-item-count"><?php echo $cnt; ?> members</span>
                            <span class="group-item-check <?php echo $has_inout ? 'check-ok' : 'check-missing'; ?>">
                                <?php echo $has_inout ? 'IN/OUT' : 'No IN/OUT'; ?>
                            </span>
                        </div>
                    </div>
                    <?php endforeach; ?>
                ...
                <!-- IN/OUT notice for groups missing the columns -->
                <?php
                $needs_migration = false;
                foreach($groups as $g){
                    $cc = $conn->query("SHOW COLUMNS FROM `".$g['attendance_table']."` LIKE 'in_time'");
                    if($cc && $cc->num_rows === 0){ $needs_migration=true; break; }
                }
                if($needs_migration):
                ?>
                <div class="inout-notice" style="margin-top:14px;">
                    <div class="inout-notice-icon">ℹ️</div>
                    <p>Some groups are missing IN/OUT columns. Run the SQL migration in HeidiSQL to enable working hours tracking for all groups.</p>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div class="info-card">
                ...
                <div class="create-group-box" id="createGroupBox">
                    ...
                    <div class="inout-notice" style="margin-top:10px;">
                        <div class="inout-notice-icon">✅</div>
                        <p>New groups automatically get IN/OUT time and working hours tracking built in.</p>
                    </div>
                </div>
                <?php else: ?>
